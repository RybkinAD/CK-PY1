...
def to_csv_file(filename, headers, rows, delimeter=',', new_line='\n'):
    with open(filename, 'w') as f:
        f.writelines(f'{header}{delimeter if headers.index(header) in range(len(headers) - 1) else new_line}'
                     for header in headers)
        f.writelines(f'{value}{delimeter if _.index(value) in range(len(_) - 1) else new_line}'
                     for _ in rows for value in _)
